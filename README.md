...
const PLANETS=[
  {name:'Mercury', au:0.387, period:0.241, ecc:0.206, color:'#a8a8a8', r:2.5,  rings:false, moons:0},
  {name:'Venus',   au:0.723, period:0.615, ecc:0.007, color:'#e8c97a', r:4.5,  rings:false, moons:0},
  {name:'Earth',   au:1.000, period:1.000, ecc:0.017, color:'#4a90d9', r:5,    rings:false, moons:1, moonDist:8, moonAngle:0},
  {name:'Mars',    au:1.524, period:1.881, ecc:0.093, color:'#c1440e', r:3.5,  rings:false, moons:0},
  {name:'Jupiter', au:2.6,   period:11.86, ecc:0.049, color:'#c8863a', r:11,   rings:false, moons:0},
  {name:'Saturn',  au:3.3,   period:29.46, ecc:0.057, color:'#e4d191', r:9,    rings:true,  moons:0},
  {name:'Uranus',  au:3.9,   period:84.01, ecc:0.046, color:'#7de8e8', r:6.5,  rings:false, moons:0},
  {name:'Neptune', au:4.4,   period:164.8, ecc:0.010, color:'#3f54ba', r:6,    rings:false, moons:0},
];
...
const STARS=[];
for(let i=0;i<220;i++){
  STARS.push({x:Math.random()*W,y:Math.random()*H,s:Math.random()*1.3+0.2,ph:Math.random()*6.28,mag:Math.random()*0.7+0.3});
}
...
for(let i=0;i<100;i++){
  ...
  ASTEROIDS.push({a,d,da:(Math.random()-0.5)*0.00025,s:Math.random()*0.7+0.2});
}
...
function getPlanetXY(p){
  ...
  return{x:CX+a*Math.cos(p.angle)-a*p.ecc, y:CY+b*Math.sin(p.angle)};
}

function drawStars(){
  STARS.forEach(s=>{
    s.ph+=0.008;
    const a=(0.3+Math.sin(s.ph)*0.22)*s.mag;
...
function drawShoots(){
  shootTimer++;
  if(shootTimer>100&&Math.random()<0.5){
    shootTimer=0;
    SHOOTS.push({x:Math.random()*W*0.6,y:Math.random()*H*0.35,vx:Math.random()*4+3,vy:Math.random()*2+1,life:1,len:30+Math.random()*25});
  }
  for(let i=SHOOTS.length-1;i>=0;i--){
    const s=SHOOTS[i];
    s.x+=s.vx;s.y+=s.vy;s.life-=0.04;
    if(s.life<=0||s.x>W){SHOOTS.splice(i,1);continue;}
    const g=cx.createLinearGradient(s.x-s.vx*6,s.y-s.vy*6,s.x,s.y);
    g.addColorStop(0,'rgba(255,255,255,0)');
    g.addColorStop(1,`rgba(255,255,255,${s.life*0.9})`);
    cx.beginPath();cx.moveTo(s.x-s.vx*6,s.y-s.vy*6);cx.lineTo(s.x,s.y);
...
function drawOrbits(){
  PLANETS.forEach(p=>{
    const a=p.au*AU;
    const b=a*Math.sqrt(1-p.ecc*p.ecc);
    cx.save();cx.translate(CX-a*p.ecc,CY);
    cx.beginPath();cx.ellipse(0,0,a,b,0,0,6.28);
    cx.strokeStyle='rgba(255,255,255,0.07)';cx.lineWidth=0.5;cx.stroke();
    cx.restore();
  });
}
...
function drawAsteroids(){
  ASTEROIDS.forEach(a=>{
    a.a+=a.da;
    const x=CX+Math.cos(a.a)*a.d-(AU*2.27)*0.049;
    const y=CY+Math.sin(a.a)*a.d*0.93;
    cx.beginPath();cx.arc(x,y,a.s,0,6.28);
    cx.fillStyle='rgba(180,160,130,0.4)';cx.fill();
  });
}

function drawSun(){
  for(let i=3;i>=0;i--){
    cx.beginPath();cx.arc(CX,CY,16+i*8,0,6.28);
    cx.fillStyle=`rgba(255,${200-i*20},${30},${0.07-i*0.014})`;cx.fill();
  }
  for(let i=0;i<10;i++){
    const a=(i/10)*6.28+frame*0.003;
    const r2=24+Math.sin(frame*0.05+i)*3;
    cx.beginPath();cx.moveTo(CX+Math.cos(a)*16,CY+Math.sin(a)*16);
    cx.lineTo(CX+Math.cos(a)*r2,CY+Math.sin(a)*r2);
    cx.strokeStyle='rgba(255,220,80,0.14)';cx.lineWidth=1.2;cx.stroke();
  }
  cx.beginPath();cx.arc(CX,CY,15,0,6.28);
  cx.fillStyle='#fff4a0';cx.fill();
  cx.fillStyle='#fffde0';cx.font='bold 6px monospace';
  cx.textAlign='center';cx.fillText('SOL',CX,CY+2.5);
}
...
function drawPlanets(){
  PLANETS.forEach((p,i)=>{
    ...
    if(p.rings){
      ...
      cx.beginPath();cx.arc(0,0,p.r+9,0,6.28);
      cx.strokeStyle='rgba(228,209,145,0.55)';cx.lineWidth=4;cx.stroke();
      cx.beginPath();cx.arc(0,0,p.r+5,0,6.28);
      cx.strokeStyle='rgba(200,180,110,0.3)';cx.lineWidth=2;cx.stroke();
      cx.restore();
    }

    cx.beginPath();cx.arc(x,y,p.r+4,0,6.28);
    cx.fillStyle=`rgba(${rr},${gg},${bb},0.12)`;cx.fill();
    cx.beginPath();cx.arc(x,y,p.r,0,6.28);
    cx.fillStyle=p.color;cx.fill();

    if(i===2){
      cx.save();cx.beginPath();cx.arc(x,y,p.r,0,6.28);cx.clip();
      cx.fillStyle='rgba(34,100,60,0.5)';cx.fillRect(x-p.r,y-0.5,p.r*0.6,p.r*0.8);
      cx.fillStyle='rgba(255,255,255,0.15)';cx.fillRect(x-p.r,y-p.r*0.3,p.r*2,p.r*0.22);
      cx.restore();
    }
    if(i===4){
      cx.save();cx.beginPath();cx.arc(x,y,p.r,0,6.28);cx.clip();
      cx.fillStyle='rgba(180,120,60,0.35)';
      cx.fillRect(x-p.r,y-2.5,p.r*2,3.5);cx.fillRect(x-p.r,y+4,p.r*2,2.5);
      cx.beginPath();cx.ellipse(x+3,y+4,2.5,1.5,0,0,6.28);
      cx.fillStyle='rgba(200,80,50,0.55)';cx.fill();
      cx.restore();
    }

    cx.beginPath();cx.arc(x,y,p.r,0,6.28);
    cx.strokeStyle=`rgba(${rr},${gg},${bb},0.45)`;cx.lineWidth=0.5;cx.stroke();

    if(p.moons){
      p.moonAngle+=0.022;
      const mx=x+Math.cos(p.moonAngle)*p.moonDist;
      const my=y+Math.sin(p.moonAngle)*p.moonDist*0.55;
      cx.beginPath();cx.arc(mx,my,1.5,0,6.28);
      cx.fillStyle='#cccccc';cx.fill();
    }

    cx.fillStyle='rgba(200,225,210,0.65)';
    cx.font='7px monospace';cx.textAlign='center';
    cx.fillText(p.name,x,y-p.r-4);
  });
}

function drawHUD(){
  cx.fillStyle='rgba(0,10,5,0.65)';cx.fillRect(6,6,170,38);
  cx.strokeStyle='rgba(100,180,255,0.2)';cx.lineWidth=0.5;cx.strokeRect(6,6,170,38);
  cx.fillStyle='#88ccff';cx.font='bold 8px monospace';cx.textAlign='left';
  cx.fillText('SOLAR SYSTEM — REAL ORBITS',12,18);
  cx.fillStyle='rgba(150,200,255,0.6)';cx.font='7px monospace';
  cx.fillText('8 PLANETS  •  KEPLER MECHANICS  •  ASTEROID BELT',12,32);
}

function tick(){
  cx.clearRect(0,0,W,H);
  cx.fillStyle='#000008';cx.fillRect(0,0,W,H);
  drawStars();drawShoots();drawOrbits();drawAsteroids();drawSun();drawPlanets();drawHUD();
  const ts=0.00065;
  PLANETS.forEach(p=>{p.angle+=((2*Math.PI)/p.period)*ts;});
  frame++;
